...
class ContactusFormRepository implements IContactusFormRepository
{
    ...
    public function connect_api(array $data)
    {
        ...
        'field[39]'              => $data['message'],
        'first_name'             => $data['full_name'],
        'field[21]'              => $data['full_name'],

            'status[123]'              => 1, // 1: active, 2: unsubscribed (REPLACE '123' WITH ACTUAL LIST ID, IE: status[5] = 1)
            'form'          => 9, // Subscription Form ID, to inherit those redirection settings
        );
...
// Result info that is always returned
    ...
    {
        Mail::send('emails.send-contact', ['data' => $data], function ($message) use ($data) {
           // $message->to('info@autofetch.co.nz')
               $message->to('dani@autofetch.co.nz')
                    ->from('nautofetch@gmail.com')
                ->subject('Autofetch Contact us');
        });
        //return $this->connect_api($data);
    }
